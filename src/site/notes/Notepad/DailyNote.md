...
- 中集旗下的安瑞科于2011年耗资1.65亿收购扬子石化设计院，即现在的中集工程科技，总部位于南通。
- 石油行标(SY)比石化行标(SH)要求更高，造价高10%。
- 安监总局（现已并入应急管理部）于2016年发文将仪表分为3个系统：DCS、SIS、GDS。
- 江苏地区SIS要求高，建议由第三方出具HAZOP和LOPA报告，指导SIL定级。石化仪表通常为SIL2和SIL3，SIL4用于列车、航空、石化电力。
- 工艺提给仪表的条件：
	1. 管径：用于确定仪表套管；
	2. 环境：用于仪表选型；
	3. 变量：用于确定精度。
	4. 流量计应根据压损确定孔板选型，控制阀需要确定其扭矩。
- 仪表设计流程：
	1. 方案：提供初步P&I图，主要是工艺流程的规划，仪表部分仅供参考；
	2. 施工图：根据HAZOP和LOPA报告确定SIL等级，完成详细的P&I图，确定机柜间、配电间，提供可燃气体探测点位用于图审。
	3. 详细设计：机房平面、线缆桥架、GDS点位、规格书、电缆表、I/O表、逻辑图、接线图、安装图。
...
- 洁净室泪珠灯有18/27/54W、0.6/0.9/1.2m可选，常用的选型是27W、1.2m。
- 洁净灯盘吸顶安装，有600×600、600×1200可选。
- 黄光灯的功率一般是16W，可定制48W。
- 仓储的灯具可自带微波探测器，叉车经过时开启照明。
- 食品加工洁净室采用IP54防潮灯具。
- 洁净灯具有Ⅰ类也有Ⅱ类，电源外置则为Ⅰ类。

2024-03-15

### 设计事业部总师沟通会

- 设计部与财务部协调好收费的财务事项，避免财务报表与实际相差过大。
- 项目总师应熟悉合同，明确合同范围和收费节点，明确什么时候该收多少钱，如实上报，及时提供审计依据。
- 特殊情况处理建议：满足收费条件却因甲方原因未能及时付费，暂缓开票；汇款至总院时，及时与财务、甲方协商，提供汇款依据，避免不好确定是否为南京分院款项。
...
### 山东石油（中石化分公司）章丘中央仓设计方案 #unfinished 

- 仓储面积近1万平（包括室外堆场）,仓储物品包括丙一类的白酒、润滑油。
1万平仓库的租金近300
...
### 《建筑物移动通信基础设施建设标准 DB 32/4120-2021》
...
### 芯长征项目站房布置
 ...
 - 厂房各专业仍有意见：
	- 水专业提出报警阀过大；
	- 电气提出甲类库可参照百济在安全玻璃后加一层泄爆墙，另外考虑低压配电箱不放在变电站内（变电站由东方院设计），一层需要增加配电间；
	- 暖通提出平面的房间分隔不合理，根据以往和东方院接触的经验，我们应谨慎确定布局，减少后期改动，建议隔墙全部取消，重新布置站房。
- 最终站房全部重新布置，配电间和弱电间改为居中布置并上下贯通，报警阀间、风井重新规划，尽量将隔间扩大，工艺设计时可以充分利用，用于安装动力、管道等设备。

2022-10-27

### 智能化设计资质

2006年，设立四个专项的设计和施工一体化资质，包括智能化、消防、装饰、幕墙。2015年，为提高设计的专业性，设立专项设计资质，当时的设计施工一体化资质可“换证”，分拆成设计和施工两个资质。 
...
- 原设计参照博康品牌，设置迷你型空气采样，可直接接入火警，不需要配备电源。
- 深化设计未采用该品牌，将迷你型空气采样换成普通型。另外，在所有风口处增加了毛细支管。这些改动都将牵涉到造价的增加。
- 设计院在技术上认可深化的方案，但是需要说清楚增加造价并不是因为原设计不满足要求。|

2022-10-20

### 冷库电气施工防止产生冷桥的措施：

1. 穿过冷间保温层的电气线路应相对集中敷设，预先留出走管孔洞，防止因穿墙打洞破坏隔热层。
2. 预留墙洞四周用木板封闭成盒状，并做好隔汽层，待建设后，用软木或软木屑拌沥青填实。
3. 上述做法供参考，冷库厂家深化设计时需确保穿过冷间的电气线路采取防火和防止产生冷桥的措施。

###  供电公司设计院名录

虽然南京分院满足资质要求，并且配备了相关设计人员，但是不在南京供电公司的设计院名录内。芯长征项目的变电站设计将交由东方院设计。 

2022-10-21

晶澳科技将在扬州开发区投入二十多亿，建设10GW电池组件项目。

2022-10-18

### 估算芯片厂房的火警点数

每50m²预计有4个点位（吊顶下、静压箱、屋顶的烟感，以及一个联动点位），因此可按每10平方1个点位估算，1万平的单层芯片厂房大致1000个点位。

### 残卫呼叫系统的做法

残卫内安装3个按钮，分别为低位呼救（0.5m）、高位呼救（1.0m）、复位（1.8m），这些按钮以及报警器连接到暗装的控制单元，再接到显示终端。显示终端配单相电，通过信号模块反馈信号至消控室。
...
- 由于没按常规做法分包，施工流程出现了问题，M1的烘箱间设备和W1/W2高架库的货架已经安装了，房间顶部的空气采样还未安装，造成施工困难。  
- 现场人员认为高架库的空气采样安装在侧墙回风处即可，顶部无需安装。

### 电缆排管

如今电缆更多地采用排管敷设，排管分为明挖、拉管、顶管等，厂区内多为明挖，开挖后夯实沟底，放好混凝土底板，将排管用管枕固定放入，再回填沙土。如果有承重要求，排管可以做混凝土包封。
...
-  设计院在智能化方案阶段应明确的内容包括：系统架构、设备要求、布置原则、安装方式、布线要求、电源要求等。
- AGV(Automated Guided Vehcle)智能搬运机器人对AP部署有明确的要求，包括信号覆盖范围、信号强度、STA漫游策略等。
- 对于高端厂房，综合布线提高要求，网线的最远距离可按75m考虑。

### 锂电池项目的二次配

- 锂电池设备的二次配包括：配电、排风、冷水、纯水、废水、压缩空气、氮气、真空、电解液、NMP等。

2022-08-17

- [x] 回复中航锂电关于M1缺少烟感以及室外增加手孔井的问题
- [x] 回复中建五局关于中润小子项的最终意见并修改到位
- [x] 发出中复神鹰投标的技术标方案文字
...
### 中航锂电项目概况
  ...
  2. 10月20提交M1的土建和消防的招标图纸，电气的设计内容包括：
...
- 智能化厂家朗高的吴晓磊来分院，汇科建设周总推荐的厂家，提供他们初步设计和点位统计，对方说周总要求一个-之内完成。
...
-  普洛斯工作范围：淮安普洛斯 A1 A2 室外；- 天津普洛斯 A2 A3 A4 室外。
...
- 线型光束感烟
	   ...
	   3. 有对流层另作考虑
	   4. 分层交错布置
-  管路吸气感烟：至少2个采样孔在16m以下（有对流层另作考虑），回风口探测辅助报警
